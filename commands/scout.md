...
`fit`, `stretch` or `unqualified` — plus the plain-English reasons, in `why`. Then it writes
...
as a suggestion. To change what counts as a match, send them to `/recruit:goals` — not to
